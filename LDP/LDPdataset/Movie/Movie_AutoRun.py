...
def LDP(n,l,k,total_eps,L):
    ...
    # 数据路径定义
    root_data_path = "../Movie/data/"
    # 结果路径定义
    root_result_path = "../Movie/run/" + str(total_eps) + "/"
    if not os.path.exists(root_result_path):
        os.makedirs(root_result_path)
        print("路径" + root_result_path + "创建成功")

    k_path = root_data_path + "Movie_k.txt"
    v_path = root_data_path + "Movie_v.txt"
    samplek_path = root_data_path + "Movie_samplek.txt"
    ...
    lable_path = root_data_path + 'Movie_id_count_avg.txt'
    ...
    filename = root_result_path + 'Movie_result_' + str(total_eps) + '_' + s + '.csv'
    bf.savecsv(np.array(id_tcectaea), filename)


if __name__ == '__main__':
    LDP(n=138493,l=26744,k=10,total_eps=6,L=25)
