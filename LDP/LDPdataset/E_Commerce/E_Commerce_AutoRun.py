...
def LDP(n,l,k,total_eps,L):
    ...
    # 数据路径定义
    root_data_path = "../E_Commerce/data/"
    # 结果路径定义
    root_result_path = "../E_Commerce/run/" + str(total_eps) + "/"
    if not os.path.exists(root_result_path):
        os.makedirs(root_result_path)
        print("路径" + root_result_path + "创建成功")

    k_path = root_data_path + "Ecomm_k.txt"
    v_path = root_data_path + "Ecomm_v.txt"
    samplek_path = root_data_path + "Ecomm_samplek.txt"
    ...
    lable_path = root_data_path + 'Ecomm_id_count_avg.txt'
    ...
    for i in range(2 * k):
        ...
        id_tcectaea.append((index, tmp[0], tmp[1], n * f_k[i], tmp[2], m_k[i]))

    # 获取时间作为文件名后缀
    s = time.strftime('%m%d_%H%M%S', time.localtime(time.time()))
    filename = root_result_path + 'Ecomm_result_' + str(total_eps) + '_' + s + '.csv'
    bf.savecsv(np.array(id_tcectaea), filename)


if __name__ == '__main__':
    LDP(n=23486,l=1206,k=20,total_eps=20,L=1)
