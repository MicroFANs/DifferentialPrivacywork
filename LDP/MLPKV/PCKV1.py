...
def PCKV_GRR(param):
    ...
    PS=[pcb.P_S(kv[i],d,padding_length) for i in range(n)]

    # eps1=np.log((padding_length*(np.exp(epsilon)-1))/2+1)
    # eps2=np.log(padding_length*(np.exp(epsilon)-1)+1)
    # a1=pcb.p_values(eps1,n=(d+padding_length))
    # p1=pcb.p_values(eps2)
    # b1=(1-a1)/(d+padding_length-1)

    a=(padding_length*(np.exp(epsilon)-1)+2)/(padding_length*(np.exp(epsilon)-1)+2*(d+padding_length))
    b=(1-a)/(d+padding_length-1)
    p=(padding_length*(np.exp(epsilon)-1)+1)/(padding_length*(np.exp(epsilon)-1)+2)


    pad_item=[i+1 for i in range(d,d+padding_length)]
    pad_label=label+pad_item
    perturbed_GRR=pcb.PCKV_GRR_mutilprocess(PS,d,padding_length,pad_label,a,p,b)

    f_k,m_k=pcb.AEC_GRR(perturbed_GRR,d,padding_length,n,a,p,b)
    ...
    # 保存结果
    save_path = '../MLPKV/result/PCKV_Ecomm_result.txt'
